...
{ [Flags] public enum Flag { None=0, Confused=1, Stunned=2, Hallucinating=4, Asleep=8 }
  ...
  { Point np = Global.Move(Position, dir);
    ...
    { Tile t = Map[np];
      ...
      { int damage = w==null ? CalculateDamage() : w.CalculateDamage(this);
        if(damage>=10)
        { App.IO.Print("Crash! You break down the door.");
          Map.SetType(np, TileType.RoomFloor);
        }
        else App.IO.Print("Wham!");
        LoudNoise();
      }
      else if(!Map.IsPassable(t.Type))
      { App.IO.Print("Wham!");
        LoudNoise();
      }
      else App.IO.Print("You swing at thin air.");
    }
  }
  ...
  public void Die(Entity c) { Die(c.aName); }
  ...
  { int x2 = creature.Position.X-X, y2 = creature.Position.Y-Y;
  ...
  static public Entity Generate(Type type, int level) { return Generate(type, level, CreatureClass.RandomClass); }
  static public Entity Generate(Type type, int level, CreatureClass myClass)
