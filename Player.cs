...
public sealed class Player : Entity
{
      ...
      case Action.DropType:
        {
          ...
          else
          {
            ...
            menuItems.Add(new MenuItem("All types", 'a'));
            char c = 'b';
            for(int i=0; i<(int)ItemType.NumTypes; c++, i++)
            {
              if(Inv.Contains((ItemType)i)) { menuItems.Add(new MenuItem(((ItemType)i).ToString(), c)); break; }
            }
            menuItems.Add(new MenuItem("Auto-select every item", 'A'));

            {
              ...
              if(knownBlessed) menuItems.Add(new MenuItem("Items known to be blessed", 'B'));
              if(knownCursed) menuItems.Add(new MenuItem("Items known to be cursed", 'C'));
              if(unpaid) menuItems.Add(new MenuItem("Unpaid items", 'P'));
              if(knownUncursed) menuItems.Add(new MenuItem("Items known to be uncursed", 'U'));
              if(unknown) menuItems.Add(new MenuItem("Items of unknown B/C/U status", 'X'));
            }

            items = App.IO.Menu(menuItems.ToArray(), MenuFlag.Multi);
            ...
            {
              if(char.IsLower(item.Char)) // lowercase letters indicate // TODO: indicate what??
              {
                if(item.Char!='a') itemTypes.Add((ItemType)(item.Char-'a'-1)); // convert letters back to item classes
            ...
            if(itemTypes.Count>0) // if some item classes were selected, add those items to 'inv'
            {
              foreach(Item i in Inv.GetItems(itemTypes.ToArray())) inv.Add(i);
            }
  ...
  /// <summary>The player's memory of the current map, or null if he has no memory of it.</summary>
  public Map Memory;
  /// <summary>A sorted list of indices of the player's known item classes.</summary>
  public List<int> Knowledge = new List<int>();
  /// <summary>The player's pool of points for exercising skills and attributes.</summary>
  public int ExpPool;
  /// <summary>The player's original race. (The current race may be different due to polymorph, etc.)</summary>
  ...
  bool GroundPackUse(string verb, out Item item, out IInventory inv, params ItemType[] types)
  {
    ...
    {
      if(!Inv.Contains(types)) { App.IO.Print("You have nothing to {0}!", verb.ToLower()); return false; }
