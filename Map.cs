...
namespace Chrono
{
  ...
  [Flags]
  public enum TileFlag : byte { None=0, Hidden=1, Locked=2, Seen=4 };
  ...
  { // maximum sent on a tile; maximum scent taht can be added at once (maximum entity stench); maximum sound on a tile
    ...
    public sealed class EntityCollection : Collection<Entity>
    {
      public EntityCollection(Map map) { this.map = map; }

      protected override void ClearItems()
      {
        foreach(Entity e in this) map.OnRemove(e);
        base.ClearItems();
      }
    ...
    public Link GetLink(Point pt, bool autoGenerate)
    {
      ...
      {
        if(links[i].FromPoint==pt) return GetLink(i, autoGenerate);
      }
    ...
    public Room GetRoom(Point pt)
    {
      ...
      {
        if(rooms[i].OuterArea.Contains(pt)) return rooms[i];
      }
      return null;
    }
    public Room GetRoom(string id)
    {
      for(int i=0; i<rooms.Length; i++)
      {
        if(rooms[i].Name==id) return rooms[i];
      }
      return null;
    }
    ...
    public void MakeNoise(Point center, Entity source, Noise type, byte volume)
    {
      ...
      {
        for(int x=0; x<Width; x++) map[y, x].Sound = 0; // clear existing sound values
      }
    ...
    */
      ...
      for(int y=0; y<Height; y++)
      {
        for(int x=0; x<Width; x++)
        {
          ...
          if(pile!=null)
          {
            ...
            {
              foreach(Item i in toRemove) pile.Remove(i);
              toRemove.Clear();
            }
          }
        }
    ...
    Point FindXmlLocation(XmlNode node, IDictionary<string,Link> links) { return FindXmlLocation(node, links, false); }
    Point FindXmlLocation(XmlNode node, IDictionary<string, Link> links, bool optional)
    {
    ...
    ushort[,] scentmap;
    string mapID, name;
    Queue<Entity> thinkQueue = new Queue<Entity>();
  ...
  public sealed class ShopType
  {
    ...
    static ShopType()
    {
      ...
      ShopType.types = new SortedList<string,ShopType>(types.Length);
      for(int i=0; i<types.Length; i++) ShopType.types.Add(types[i].ID, types[i]);
    ...
    {
      ShopType st;
      if(!types.TryGetValue(id, out st)) throw new ArgumentException("No such shop type: "+id);
      return st;
    }
    ...
    public static ShopType GetRandom(int numSquares)
    {
      ...
      while(true)
      {
        ShopType st = types.Values[typeIndex];
        if(++typeIndex == types.Count) typeIndex = 0;
        left -= st.Chance;
        if(left<0) return st.IsTooBig(numSquares) ? types.Values[fallback] : st;
      }
    }
    ...
    static SortedList<string,ShopType> types;
